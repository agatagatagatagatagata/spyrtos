...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const start = async () => {
	...
	// Load the a marker 3D model (key)
    const amarker = await loadGLTF('./assets/models/the_halloween_pirate_skull/scene.gltf');
    amarker.scene.scale.set(0.4, 0.4, 0.4);
    amarker.scene.position.set(0, 0, 0);
	amarker.scene.rotation.set(0, 0, 0);
	
	// Load the a marker 3D model (key)
    const a1marker = await loadGLTF('./assets/models/the_halloween_pirate_skull/scene.gltf');
    a1marker.scene.scale.set(0.4, 0.4, 0.4);
    a1marker.scene.position.set(0, 0, 0);
	a1marker.scene.rotation.set(0, 0, 0);
	
	// Load the b marker 3D model  (pirate)
	const bmarker = await loadGLTF('./assets/models/stylized_key/scene.gltf');
    bmarker.scene.scale.set(0.1, 0.1, 0.1);
    bmarker.scene.position.set(0, 0, 0);
	bmarker.scene.rotation.set(0, 0, 45);
	...
	const a1markerAnchor = mindarThree.addAnchor(3);
    a1markerAnchor.group.add(a1marker.scene);
		// marker b anchor
	const bmarkerAnchor = mindarThree.addAnchor(4);
